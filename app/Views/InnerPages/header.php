<header class="bg-white border-b border-gray-300">
    ...
    <script>
        function toggleMenu() {
            var dropdown = document.getElementById('toggle');
    ...
    <div class="container mx-auto p-4 flex items-center justify-between">
        ...
        <button class="lg:hidden bg-transparent  text-orange-500 font-semibold  py-2 px-4 border border-orange-500 rounded" onclick="toggleMenu()" id="dropdownDefaultButtontoggle" data-dropdown-toggle="toggle">
            &#9776;
        </button>
        ...
        <div id="dropdown" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
            <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                <li>
                    <a href="<?= base_url('/reclame') ?>" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 ">Add Reclamation</a>
                </li>
                <li>
                    <a href="<?= base_url('/list-reclame') ?>" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 ">List of Reclamation</a>
                </li>
            </ul>
        </div>
        <?php
        if (session()->has('PseudoNom') && !empty(session('PseudoNom'))) {
           ...
           <button class="menu-item py-2 px-4 text-gray-800  " id="dropdownDefaultButton" data-dropdown-toggle="dropdown" type="button">Hello ' . session()->get('PseudoNom') . '</button>
            ...
            echo ' <ul  class="hidden lg:flex space-x-4">
            <li class="menu-item border-2 border-orange-500 hover:bg-orange-500 rounded-md py-2 px-4 text-gray-800  "><a href="' . base_url('sign-in') . '" class="">Sign In</a></li>
            <li class="menu-item border-2 border-orange-500 hover:bg-orange-500 rounded-md py-2 px-4 text-gray-800  "><a href="' . base_url('sign-up') . '" class="">Sign Up</a></li>
        </ul>';
        } ?>
    </div>
    <!-- Mobile phone -->
    <div id="toggle" class="z-4 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
